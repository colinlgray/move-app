...
import { leaderboardIdlTwo, plantsProgramId } from "../../hooks/useProgram";
...
export const submitGoods = async ({
  ...
  async function createTransaction(amount: number, plant: PlantSource) {
    ...
    const [plantKey] = await PublicKey.findProgramAddress(
      [Buffer.from("plant"), plant.plantMintPubKey.toBuffer()],
      new PublicKey(plantsProgramId)
    );
